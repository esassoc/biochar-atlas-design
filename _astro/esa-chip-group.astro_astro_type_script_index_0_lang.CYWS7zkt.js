import{i as h,b as l,a as p}from"./lit-element.C8p3bJxG.js";class d extends h{constructor(){super(),this.onKeydown=e=>{const r=this.options;if(!r||r.length===0)return;const t=this.renderRoot.querySelectorAll(".chip"),o=Array.from(t).indexOf(this.renderRoot.activeElement),i=this.multiple?Math.max(0,o):Math.max(0,r.findIndex(u=>u.value===this.value));let a;switch(e.key){case"ArrowRight":case"ArrowDown":a=(i+1)%r.length;break;case"ArrowLeft":case"ArrowUp":a=(i-1+r.length)%r.length;break;case"Home":a=0;break;case"End":a=r.length-1;break;case"Enter":case" ":e.preventDefault(),this.select(r[i]);return;default:return}e.preventDefault(),this.multiple||this.select(r[a]),t[a]?.focus()},this.options=[],this.value="",this.values=[],this.multiple=!1,this.size="md",this.name="",this.label="",this.internals=this.attachInternals()}static{this.formAssociated=!0}static{this.properties={options:{type:Array},value:{type:String,reflect:!0},values:{type:Array},multiple:{type:Boolean,reflect:!0},size:{type:String,reflect:!0},name:{type:String},label:{type:String}}}willUpdate(e){if(e.has("options")&&typeof this.options=="string")try{this.options=JSON.parse(this.options)}catch{this.options=[]}if(e.has("values")&&typeof this.values=="string")try{this.values=JSON.parse(this.values)}catch{this.values=[]}}connectedCallback(){super.connectedCallback(),this.internals.role=this.multiple?"group":"radiogroup",this.label&&(this.internals.ariaLabel=this.label),this.syncFormValue()}updated(){this.label&&(this.internals.ariaLabel=this.label)}syncFormValue(){this.multiple?this.internals.setFormValue(this.values.length?this.values.join(","):null):this.internals.setFormValue(this.value||null)}isActive(e){return this.multiple?this.values.includes(e.value):e.value===this.value}select(e){if(this.multiple){this.values=this.values.includes(e.value)?this.values.filter(r=>r!==e.value):[...this.values,e.value],this.syncFormValue(),this.dispatchEvent(new CustomEvent("change",{detail:{values:[...this.values]},bubbles:!0,composed:!0}));return}e.value!==this.value&&(this.value=e.value,this.syncFormValue(),this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value},bubbles:!0,composed:!0})))}render(){return l`
      <div class="root" @keydown=${this.onKeydown}>
        ${(this.options??[]).map((e,r)=>{const t=this.isActive(e),o=this.multiple?r===0:t;return l`
            <button
              type="button"
  ...
  `}}customElements.get("esa-chip-group")||customElements.define("esa-chip-group",d);const n=[{value:"water",label:"Water retention"},{value:"som",label:"Soil organic matter"},{value:"carbon",label:"Carbon sequestration"},{value:"ph",label:"pH adjustment"}],c=[{value:"neutral",label:"Neutral"},{value:"neutral-strong",label:"Neutral strong",tone:"neutral-strong"},{value:"brand",label:"Brand",tone:"brand"},{value:"amber",label:"Amber",tone:"amber"}];customElements.whenDefined("esa-chip-group").then(()=>{document.querySelectorAll('esa-chip-group[data-demo="goals"]').forEach(r=>{r.options=n});const s=document.querySelector('esa-chip-group[data-demo="goals-multi"]');s&&(s.options=n,s.values=["water","som","carbon"]);const e=document.querySelector('esa-chip-group[data-demo="tones"]');e&&(e.options=c,e.values=c.map(r=>r.value))});
